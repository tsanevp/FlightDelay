...
-- Filter F1 & F2 by cancelled, diverted, origin, destination, and ensure dates for are between 06/2007 and 05/2008
F1_Filtered_PreJoin = FILTER Filtered_Flight1 BY cancelled1 == 0 AND diverted1 == 0 AND (origin1 == 'ORD' AND dest1 != 'JFK') AND ((year1 == 2007 AND month1 >= 6) OR (year1 == 2008 AND month1 <= 5));
F2_Filtered_PreJoin = FILTER Filtered_Flight2 BY cancelled2 == 0 AND diverted2 == 0 AND (origin2 != 'ORD' AND dest2 == 'JFK') AND ((year2 == 2007 AND month2 >= 6) OR (year2 == 2008 AND month2 <= 5));
